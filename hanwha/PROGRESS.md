...
한화 단일 개인 앱에서 **KBO 10구단 티켓팅 도우미**로 전환한다. 무료 PWA로 예매 캘린더와 알림 저장 수요를 검증한 뒤, 지표가 충분할 때 네이티브 알림/제휴/스토어 배포로 확장한다.

## 원칙

- 합법 알림 도우미: 예매처 연결, 오픈 일정 정리, 알림 저장만 제공한다.
- 금지: 자동예매, 대기열/안티봇 우회, 취소표 자동 감지, 계정 로그인 대행.
...
2. 수요 검증 운영 루틴을 정한다.
3. 지표가 쌓이면 네이티브 알림 또는 제휴 링크 실험으로 확장한다.
